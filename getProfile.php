...
<div class="container">
	...
	<div class="profile">
       ...
       {     var namein=document.getElementById("name1").value;
            ...
            firebase.database().ref('/'+role+'/' +uid).update({
    name: namein ,
    emailID: emailin,
    phoneNo :contactin,
    address:addressin,
...
function fn3(role,userId)
	{
		 firebase.database().ref("/"+role+"/" + <?php echo "'".$userid."'";?>+"/Works/").once('value').then(function(snapshot) {
			snapshot.forEach(function(childSnapshot) {
    var childKey = childSnapshot.key;
    var childData = childSnapshot.val();
  $("#inner3").append("<img class='work_pic' src=\""+childData+"\" height='100px' width='150px'>");
  });
...
function fn2() {
        ...
        var fd=1;
	firebase.database().ref().child("Architects").orderByChild("emailID").equalTo(<?php echo "'".$usermail."'";?>).on("value", function(snapshot) {
if (snapshot.exists() && fd==1) {
  ...
  firebase.database().ref().child("Architects").orderByChild("Email").equalTo(<?php echo "'".$usermail."'";?>).on("value", function(snapshot) {
if (snapshot.exists() && fd==1) {
     fd=0;
    $('#role').html("Architect");
    role="Architects";
      ...
      firebase.database().ref().child("Engineers").orderByChild("emailID").equalTo(<?php echo "'".$usermail."'";?>).on("value", function(snapshot) {
if (snapshot.exists() && fd==1) {
     fd=0;
     role="Engineers";
    ...
    $('#role').html("Engineer");
      ...
      firebase.database().ref().child("Engineers").orderByChild("Email").equalTo(<?php echo "'".$usermail."'";?>).on("value", function(snapshot) {
if (snapshot.exists() && fd==1) {
     fd=0;
     role="Engineers";
    ...
    $('#role').html("Engineer");
        ...
        firebase.database().ref().child("Interior Designers").orderByChild("emailID").equalTo(<?php echo "'".$usermail."'";?>).on("value", function(snapshot) {
if (snapshot.exists() && fd==1) {
     fd=0;
    ...
    $('#role').html("Interior Designer");
        ...
        firebase.database().ref().child("Interior Designers").orderByChild("Email").equalTo(<?php echo "'".$usermail."'";?>).on("value", function(snapshot) {
if (snapshot.exists() && fd==1) {
     fd=0;
     role="Interior Designers";
    $('#role').html("Interior Designer");
        ...
        firebase.database().ref().child("Users").orderByChild("emailID").equalTo(<?php echo "'".$usermail."'";?>).on("value", function(snapshot) {
if (snapshot.exists() && fd==1) {
     fd=0;
     role="Users";
    $('#role').html("Customer");
	...
	setTimeout(function(){ fn(<?php echo "'".$userid."'";?>) },4000);
	if(role!="Users")
{  setTimeout(function(){ fn3(role,<?php echo "'".$userid."'";?>)},6000);
...
$(function(){setTimeout(fn2,3000);
	...
	<h4 class="hello"><b>Welcome to Profile of:</b></h4>
